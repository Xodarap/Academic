...
#define TAG_SHIFT 11
#define INDEX_MASK (0x07F8)
#define INDEX_SHIFT 3
#define OFFSET_MASK (0x0007)
#define TEXT_START 0
...
signed char DCACHE_TAGS[NUM_CACHE_LINES];  //track current tag
...
void WriteDMemory(unsigned int address, int value) {
  ...
  if((address|7) > LARGEST_ADDRESS_D){
    LARGEST_ADDRESS_D = (address|7);
  }
}
...
unsigned int ReadDMemory(unsigned int address) {
  ...
  if(DCACHE_TAGS[(address&INDEX_MASK)>>INDEX_SHIFT] != -1 && 
     DCACHE_TAGS[(address&INDEX_MASK)>>INDEX_SHIFT] != (address&TAG_MASK)>>TAG_SHIFT &&
     (address|OFFSET_MASK) > LARGEST_ADDRESS_D){
    LARGEST_ADDRESS_D = address|OFFSET_MASK;
  }
  DCACHE_TAGS[(address&INDEX_MASK)>>INDEX_SHIFT] = (address&TAG_MASK)>>TAG_SHIFT;
...
int main(int argc, char* argv[])
{
  ...
  if(argc == 2){
    ...
    mem_dump_filename = (char*) malloc(strlen(argv[1])+5);
    reg_dump_filename = (char*) malloc(strlen(argv[1])+4);

    strcpy(mem_dump_filename, argv[1]);
    strcpy(reg_dump_filename, argv[1]);
